...
#define TIMER_START static long int timer_n = 0; static double   timer_accumulated_total = 0; std::chrono::high_resolution_clock::time_point t0 = std::chrono::high_resolution_clock::now();
#define TIMER_STOP std::chrono::high_resolution_clock::time_point t1 = std::chrono::high_resolution_clock::now(); std::chrono::duration<double> time_span = std::chrono::duration_cast<std::chrono::duration<double>>(t1 - t0); timer_accumulated_total += time_span.count(); timer_n++;if (timer_n >= TIMER_SAMPLE_SIZE) { std::cout << "done in " << (timer_accumulated_total / timer_n) * 1000 << " ms" << std::endl; timer_n = 0; timer_accumulated_total = 0;}